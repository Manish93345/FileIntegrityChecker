...
def rotate_logs_if_needed():
    ...
    try:
        # Check if main log file exceeds max size
        if os.path.exists(LOG_FILE) and os.path.getsize(LOG_FILE) >= MAX_LOG_SIZE:
            print_and_log(f"📁 Log file reached {MAX_LOG_SIZE/1024/1024:.1f}MB, rotating logs...")
            
            # Rotate backup files
            for i in range(MAX_BACKUP_COUNT - 1, 0, -1):
                old_log = f"{LOG_FILE}.{i}"
                old_sig = f"{LOG_SIG_FILE}.{i}"
                new_log = f"{LOG_FILE}.{i+1}"
                new_sig = f"{LOG_SIG_FILE}.{i+1}"
            ...
            # Move current files to backup.1
            if os.path.exists(LOG_FILE):
                os.rename(LOG_FILE, f"{LOG_FILE}.1")
            if os.path.exists(LOG_SIG_FILE):
                os.rename(LOG_SIG_FILE, f"{LOG_SIG_FILE}.1")
            
            # Create new empty log files
            with open(LOG_FILE, 'w', encoding='utf-8') as f:
                f.write(f"{now()} - 🔄 Log rotation completed - new log file started\n")
            
            # Create signature for the rotation message
            append_log_signature(f"{now()} - 🔄 Log rotation completed - new log file started")
            
            print_and_log("✅ Log rotation completed successfully")
...
# ---------- CLI / main ----------
def main():
    global MAX_LOG_SIZE, MAX_BACKUP_COUNT
    ...
    parser.add_argument("--max-log-size", type=int, help=f"Max log file size in MB (default: {MAX_LOG_SIZE/1024/1024}MB)")
    parser.add_argument("--max-backups", type=int, help=f"Max number of backup files (default: {MAX_BACKUP_COUNT})")
    ...
    # Update log rotation settings if provided
    
    if args.max_log_size:
        MAX_LOG_SIZE = args.max_log_size * 1024 * 1024  # Convert MB to bytes
    if args.max_backups:
        MAX_BACKUP_COUNT = args.max_backups

    print_and_log("🚀 Starting Phase-2 secure monitor (fixed)...")
    print_and_log(f"📊 Log rotation: {MAX_LOG_SIZE/1024/1024}MB max size, {MAX_BACKUP_COUNT} backups")
    ...
    print(f"Log rotation: {MAX_LOG_SIZE/1024/1024}MB max size, {MAX_BACKUP_COUNT} backups")
